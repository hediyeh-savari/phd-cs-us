...
Every one has their own way of organizing files for writing papers. Below is my approach that has worked well for me over the years. If you're my student and write papers with me, please follow this approach so that I can easily find things.

\begin{enumerate}
\item Create a separate Github Repo for each \emph{project}. 
\begin{itemize}
  \item The reason is that we will likely write multiple papers for the same project (e.g., conference paper, journal paper, workshop paper, etc). Having a single repo per project makes it easier to manage and find things.
