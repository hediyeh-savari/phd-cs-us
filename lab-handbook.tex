...
\chapter{Conference Registration and Travel}


\paragraph{General Process}
\begin{itemize}
\item I can pay for your conference registration and travel in advance using the lab's P-card.  Usually we do this together for efficiency.
\item Everything else (e.g., hotel, food, local transport) you pay yourself (e.g., using your own credit card) and then submit reimbursement requests to the university. 
\begin{itemize}
    \item You need to save all receipts for reimbursement (except for meals, see below).
    \item You need to do this after you return from the conference and within 60 days of the expense date (TODO: check?).
\end{itemize}
\end{itemize}


\paragraph{Meals}
We are paid per diem for meals when we travel for conferences. 
This means you don't need to save or submit receipts for meals.  If you don't eat anything (please do not do this), you will still get the per diem amount.

The per diem rates depend on the city we are visiting and are determined by the US General Services Administration (GSA) website: \url{https://www.gsa.gov/travel/plan-book/per-diem-rates}. When you submit reimbursement requests for meals, you will select the location and the system will automatically calculate the per diem amount for you.
