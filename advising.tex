...
In most cases, especially at top confs., papers will get rejected; and in such cases we improve and resubmit and get best paper award, e.g., our compositional verification work was rejected at CAV'25 and get Research Spotlight Award at NeurIPS'25. In some rare cases, however, rebuttal can actually change reviewers' mind as shown in~\autoref{fig:rebuttal}. 
So embrace the uncertainties of the review process; treat rebuttal as an opportunity to improve our writing and work (think of them as \emph{bug reports} that make your code better), and don't worry about the rest.    
\begin{figure}
        \centering
...
As mentioned in~\autoref{q:scooping}, all our developments, even the ones that are not published yet, are open-source and publicly available in our \href{https://code.roars.dev/}{repos}. We try to have detailed instructions, etc but in most cases as code are changed  daily, we don't have the time to make them super polished and user-friendly. But we always stand by our code and try to help others who want to use it. For example, whenever people have questions or report bugs (typically submitted through our Github issue tracker), we will try to help them out and solve the issues quickly.  

Many confs. require artifact evaluation (AE) fore reproducibility, and we will do as instructed.  However, I personally do not like the idea doing AE as it is a lot of work and we don't get much out of it. The AE reviewers usually do not have the resources to run our code and reproduce our results, e.g., our work on scaling DNN verification requires powerful machines that most AE reviewers don't have access to. In some cases, the reviewers do not even have fast internet to download our VM images!

In short, while the intention of AE is good, the current practice of AE is not very effective and cause a lot of extra work and anxiety for the authors---typically the students responsible for the work. We spent years on the research, get multiple rejections, went through brutal rebuttals to get papers accepted, but still have to wait for AE reviewers---who are not even experts in our area and don't have the resources to run our code---to run our code and reproduce our results. AE might work well for typical research that intended just to be published once and then never touched again, but for our work that is continuously evolving and being improved and continuously tested (e.g., in competitions) and used by many people. But whatever, we will do the AE as required.
